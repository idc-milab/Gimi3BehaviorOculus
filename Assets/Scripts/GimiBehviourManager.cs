...
public class GimiBehviourManager : MonoBehaviour
{
    ...
    void Update()
    {
        distance = (float)Vector3.Distance(this.transform.position, HMD.position);
        ...
        else
        {
            ...
            {
                SmoothLookAt(HMD.position, transitionSpeedToStart);
                Debug.Log("_______looking to animation___________");
            }
            if (currentRunningBehavior == true) { SmoothLookAt(AnimationStart.position, transitionSpeedToStart); }
        ...
        void enableAndDisableScripts(bool? wantedState)
        {
            ...
            if (wantedState == null)
            {
                ...
                SmoothLookAt(AnimationStart.position, transitionSpeedToStart);
                this.GetComponent<GimiStalker>().enabled = false;
                this.GetComponent<GimiInterval>().enabled = false;
            ...
            if (wantedState == true)
            {
                ...
                SmoothLookAt(HMD.transform.position, transitionSpeedToStart);
                this.GetComponent<GimiInterval>().enabled = false;
                this.GetComponent<Animator>().enabled = false;
        ...
        /* distance = (float)Vector3.Distance(this.transform.position, HMD.position);
         ...
         else
         {
             ...
             if (currentRunningBehavior == true) { SmoothLookAt(HMD.position, transitionSpeedToStart, Time.deltaTime); }
     ...
     void enableAndDisableScripts(bool? wantedState)
     {
         ...
         if (wantedState == true)
         {
             ...
             this.GetComponent<Animation>().enabled = false;
             //this.GetComponent<GimiLookatToAnimation>().enabled = false;
             SmoothLookAt(HMD.transform.position, transitionSpeedToStart, Time.deltaTime);
             this.GetComponent<GimiStalker>().enabled = true;
             currentRunningBehavior = true;
             bufferBool = false;
             return;
